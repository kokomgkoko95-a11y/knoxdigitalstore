...
ADMIN_ID = 8263739354
GROQ_API_KEY = "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"
...
def ask_ai(user_text, conversation_history=None):
    ...
    system_prompt = (
        "သင်သည် Knox Digital Store၏ ဖော်ရွေပြီး စမတ်ကျသော AI Assistant ဖြစ်သည်။ "
        ...
        "❤️ Likes: 300=1,500ks | 500=2,000ks | 1,000=4,000ks | 5,000=19,000ks | 10,000=38,000ks\n"
...
def build_tt_multiselect_menu(selected_items: dict):
    ...
    for code, item in CATALOG["TT"]["items"].items():
        # REMOVED SKIP FOR JAPAN ACC AND PROMOTE ACC TO ALLOW BUYING
        emoji = item["emoji"]
        name = item["name"]
        if code in selected_items:
            ...
            total += price
            qty_display = "" if code in ["JP", "PM"] else f" x{qty}"
            label = f"✅ {emoji} {name}{qty_display} ({price:,}ks)"
        else:
            label = f"➕ {emoji} {name}"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    tutorial_text = (
        ...
        "<b>အဆင့် ③</b> - ပြသလာသော KPay နံပါတ်သို့ Ngwe လွှဲပြီး **ပြေစာ Screenshot** ကို Bot ထံ ပို့ပေးပါ။\n"
...
async def admin_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    report_text = (
        ...
        "💡 <i>User အားလုံးကို အသေးစိတ်ကြည့်ရန် /users ကိုနှိပ်ပါ</i>\n"
        "─────────────────────"
    )
...
async def admin_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("SELECT user_id, balance FROM wallets")
    rows = cursor.fetchall()
    conn.close()
    ...
    text = "👥 <b>Bot Users စာရင်း (လက်ကျန်ငွေနှင့်တကွ)</b>\n"
    text += "─────────────────────\n"
    for uid, bal in rows[:40]:  # Avoid over-length limit
        text += f"👤 User ID: <code>{uid}</code> | Balance: <code>{bal:,}</code> ks\n"
    if len(rows) > 40:
        text += f"...\nand {len(rows) - 40} more users."
    text += f"\n─────────────────────\n📊 စုစုပေါင်းအသုံးပြုသူ: <b>{len(rows)}</b> ဦး"
    ...
    # ─── TikTok Single ───
        ...
        if action == "cf":
            ...
            confirm_caption = (
                ...
                "⏰ ကြာချိန်ကတော့ ဝန်ဆောင်မှုအလိုက် စတင်တက်မှာပါခင်ဗျ\n\n"
                "🙏 Thank You So Much!\n\n"
                "🧑‍💻 Owner - @just_knox\n\n"
    ...
    # ─── TikTok Multi ───
        ...
        if action == "cfm":
            ...
            confirm_caption = (
                ...
                f"{'🔖 Order ID: <b>#' + order_id + '</b>' + chr(10) if order_id else ''}"
                "⏰ ဝန်ဆောင်မှုအလိုက် စတင်တက်မှာပါခင်ဗျ \n\n"
                "🙏 Thank You So Much!\n\n"
                "👨‍💻 Owner - @just_knox\n\n"
                "🛍️ နောက်ထပ်အသစ်ဝယ်ရန် /start ကိုနှိပ်ပါ"
            )
    ...
    # ─── Fixed-Price ───
        ...
        if action == "cfx":
            ...
            confirm_caption = (
                ...
                f"{'🔖 Order ID: <b>#' + order_id + '</b>' + chr(10) if order_id else ''}"
                f"📦 {item_name}\n\n"
                "🥳 လူကြီးမင်းရဲ့ အကောင့်ထဲကို အောင်မြင်စွာထည့်သွင်းပြီးပါပြီခင်ဗျာ💐 \n\n"
                "🙏 Thank You So Much!\n\n"
                "👨‍💻 Owner - @just_knox\n\n"
...
if __name__ == "__main__":
    ...
    app.add_handler(CommandHandler("users",  admin_users))     # REGISTERED ADMIN USER VIEW COMMAND
